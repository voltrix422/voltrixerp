...
import { Plus, X, Upload, DollarSign, User, FileText } from "lucide-react"
...
export function PettyCashAllocation({ onClose, onSave }: PettyCashAllocationProps) {
  ...
  async function submit() {
    if (!selectedStaff || !amount || !purpose) {
      toast({
    ...
    try {
      ...
      toast({
        title: "Success",
        message: `Petty cash of PKR ${amount} allocated to ${selectedStaff.name}`,
        type: "success"
      })
      ...
      <div className="w-full max-w-md rounded-xl border bg-[hsl(var(--card))] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-2 relative">
            ...
            >
              <span className={selectedStaff ? "capitalize" : "text-[hsl(var(--muted-foreground))]"}>
                {selectedStaff ? `${selectedStaff.name} - ${selectedStaff.role}` : "Choose an employee..."}
              </span>
