...
export function FinanceSalesSalaries() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <Users className="h-5 w-5 text-[#1faca6]" />
            Sales salaries & slips
          </h2>
          <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1 max-w-2xl">
            Payroll for sales agents: base salary plus commission from delivered orders in the
            selected month. Records are saved and PDF salary slips can be downloaded anytime.
          </p>
        </div>
      ...
      <p className="text-xs text-[hsl(var(--muted-foreground))]">
        Commission is calculated from delivered orders in the selected month (same rules as CRM
        sales reports). Generated by {getSession()?.name ?? "Finance"}.
      </p>
    </div>
  )
}
