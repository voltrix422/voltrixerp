  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-lg font-semibold text-neutral-900">Support Tickets</p>
          <p className="text-sm text-neutral-500">Manage customer support tickets and queries</p>
        </div>
      ...
      {tickets.length > 0 && (
        <div className="grid grid-cols-4 gap-4">
          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            <p className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Open</p>
          ...
          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            <p className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Resolved</p>
            <p className="text-2xl font-bold text-green-600 mt-1">{resolvedCount}</p>
          </div>
          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            <p className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Closed</p>
            <p className="text-2xl font-bold text-gray-600 mt-1">{closedCount}</p>
          </div>
        </div>
      )}
      ...
      {tickets.length > 0 && showFilters && (
        <div className="rounded-xl border border-neutral-200 bg-white p-4 flex flex-wrap gap-3 items-center animate-in slide-in-from-top-2">
          ...
          <span className="text-sm text-neutral-400 ml-auto">{filtered.length} of {tickets.length}</span>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border-2 border-dashed border-neutral-300 rounded-2xl bg-white">
          ...
          <p className="text-base font-semibold text-neutral-900">No tickets yet</p>
          <p className="text-sm text-neutral-500 mt-2">Create your first support ticket to get started</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-12 text-sm text-neutral-400 border border-dashed border-neutral-300 rounded-xl bg-white">No tickets match your filters.</div>
      ) : (
        <div className="space-y-2">
          {filtered.map(t => (
            <div key={t.id} onClick={() => setViewTicket(t)}
              ...
              {/* Icon */}
              <div className="h-10 w-10 rounded-full shrink-0 overflow-hidden bg-neutral-100 flex items-center justify-center">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-sm font-semibold text-neutral-900">{t.ticketNumber}</p>
                ...
                <p className="text-xs text-neutral-500 truncate">{t.customerName} · {t.customerEmail}</p>
              </div>

              <div className="text-right shrink-0">
                <p className="text-xs text-neutral-400">{formatDate(t.createdAt)}</p>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={resetForm}>
          <div className="w-full max-w-lg rounded-2xl border border-neutral-200 bg-white overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200 shrink-0">
              <p className="text-base font-semibold text-neutral-900">{editingTicket ? "Edit Ticket" : "New Support Ticket"}</p>
            ...
            <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-neutral-700">Customer Email *</label>
      ...
      {viewTicket && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setViewTicket(null)}>
          <div className="w-full max-w-2xl rounded-2xl border border-neutral-200 bg-white overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200 shrink-0">
              <div className="flex items-center gap-3">
                <Ticket className="h-5 w-5 text-[#1a9f9a]" />
                <p className="text-base font-semibold text-neutral-900">{viewTicket.ticketNumber}</p>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8 text-neutral-500 hover:text-neutral-900" onClick={() => setViewTicket(null)}><X className="h-5 w-5" /></Button>
              </div>
            </div>
            <div className="overflow-y-auto p-6 space-y-6">
              {/* Subject */}
              <div>
                <p className="text-sm font-medium text-neutral-500 uppercase tracking-wide mb-2">Subject</p>
                <p className="text-lg font-semibold text-neutral-900">{viewTicket.subject}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-3 rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-neutral-900">{viewTicket.customerEmail}</p>
                  </div>
                </div>
                {viewTicket.customerPhone && (
                  <div className="flex items-center gap-3 rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-neutral-900">{viewTicket.customerPhone}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Description */}
              <div>
                <p className="text-sm font-medium text-neutral-500 uppercase tracking-wide mb-2">Description</p>
                <p className="text-sm text-neutral-700 whitespace-pre-wrap bg-neutral-50 rounded-lg border border-neutral-200 p-4">{viewTicket.description}</p>
              </div>

              {/* Status Change */}
              <div>
                <p className="text-sm font-medium text-neutral-500 uppercase tracking-wide mb-2">Update Status</p>
              ...
              {viewTicket.status !== "open" && (
                <div>
                  <p className="text-sm font-medium text-neutral-500 uppercase tracking-wide mb-2">Resolution Notes</p>
                  <textarea
