...
export async function GET(req: NextRequest) {
  ...
  const outlets = await prisma.erpWebsiteOutlet.findMany({
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body.id) {
      const outlet = await prisma.erpWebsiteOutlet.update({
        where: { id: body.id },
        data,
      })
    ...
    const outlet = await prisma.erpWebsiteOutlet.create({
      data: {
        ...data,
...
export async function DELETE(req: NextRequest) {
  ...
  await prisma.erpWebsiteOutlet.delete({ where: { id } })
